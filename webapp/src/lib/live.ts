...
export class Live extends EventEmmiter implements ILive {
  ...
  private set armour(v) {
    this._armour = v;
  }

  private _maxArmour: number;

  public get maxArmour() {
    return this._maxArmour;
  }

  private set maxArmour(v) {
    this._maxArmour = v;
  }

  constructor({ health, maxHealth, armour, maxArmour }: LiveData) {
    super();
